...
class PackageController extends Controller
{
    ...
    {
        $model = Page::where('Pag_Status', '!=', 2)->where('Pag_Reg_Id', getSelectedValue())->with('category')->whereHas('category', fn($query) => $query->where('PagCat_Name', 'Package'))->orderBy('Pag_CreatedDate', 'asc')->get();

        return view('backend.admin.package.index', compact('model'));
    }
    public function create()
    {
        $ImgMaxSizeModel = getImgMaxSizeModel();
        $model = PageCategory::where('PagCat_Name', 'Package')->where('PagCat_Status', '0')->orderBy('PagCat_Id', 'desc')->get();
        return view('backend.admin.package.create', compact('model','ImgMaxSizeModel'));
    }
    public function store(Request $request)
    {
        try {
            ...
            if ($request->hasFile('Pag_Image')) {
                ...
                if ($extension == 'jpg' || $extension == 'jpeg' || $extension == 'png' || $extension == 'svg' || $extension == 'webp') {
                    if ($request->file('Pag_Image')->isValid()) {
                        // Dynamically create a folder named 'package_images'
                        $subfolderName = getSelectedValue();

                        $folderName = 'Package_images';
            ...
            $model->save();
            return redirect()->route('admin.package.create')->with('success', 'Data added successfully.');
        } catch (Exception $e) {
    ...
    public function edit($hashedId)
    {
        ...
        return view('backend.admin.package.edit', compact('model','ImgMaxSizeModel'));
    }
    ...
    public function active($id)
    {
        try {
            ...
            return redirect()->route('admin.package.index')->with('success', 'Image marked as inactive.');
        } catch (\Exception $e) {
            return back()->with('error', 'Error: ' . $e->getMessage());
        }
    }
    public function inactive($id)
    {
        try {
            ...
            return redirect()->route('admin.package.index')->with('success', 'Image marked as active.');
        } catch (\Exception $e) {
            return back()->with('error', 'Error: ' . $e->getMessage());
        }
    }
