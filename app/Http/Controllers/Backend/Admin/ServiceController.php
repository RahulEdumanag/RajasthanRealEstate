...
class ServiceController extends Controller
{
    ...
    {
        $model = Page::where('Pag_Status', '!=', 2)->where('Pag_Reg_Id', getSelectedValue())->with('category')->whereHas('category', fn($query) => $query->where('PagCat_Name', 'Service'))->orderBy('Pag_SerialOrder', 'asc')->get();

        return view('backend.admin.service.index', compact('model'));
    }

    public function create()
    {
        $nextSerialOrder = getNextSerialOrder(getSelectedValue(), 'Service');
        $model = PageCategory::where('PagCat_Name', 'Service')->where('PagCat_Status', '0')->orderBy('PagCat_Id', 'desc')->get();
        $ImgMaxSizeModel = getImgMaxSizeModel();
        return view('backend.admin.service.create', compact('model', 'nextSerialOrder','ImgMaxSizeModel'));
    }
    public function store(Request $request)
    {
        try {
            ...
            $model->save();
            return redirect()->route('admin.service.create')->with('success', 'Data added successfully.');
        } catch (Exception $e) {
    ...
    public function edit($hashedId)
    {
        ...
        return view('backend.admin.service.edit', compact('model','ImgMaxSizeModel'));
    }
    ...
    public function active($id)
    {
        try {
            ...
            return redirect()->route('admin.service.index')->with('success', 'Service marked as inactive.');
        } catch (\Exception $e) {
            return back()->with('error', 'Error: ' . $e->getMessage());
        }
    }
    public function inactive($id)
    {
        try {
            ...
            return redirect()->route('admin.service.index')->with('success', 'Service marked as active.');
        } catch (\Exception $e) {
            return back()->with('error', 'Error: ' . $e->getMessage());
        }
    }
