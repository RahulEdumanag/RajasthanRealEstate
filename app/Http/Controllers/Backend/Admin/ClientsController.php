...
class ClientsController extends Controller
{
    ...
    {
        $model = Page::where('Pag_Status', '!=', 2)->where('Pag_Reg_Id', getSelectedValue())->with('category')->whereHas('category', fn($query) => $query->where('PagCat_Name', 'Clients'))->orderBy('Pag_SerialOrder', 'asc')->get();

        return view('backend.admin.clients.index', compact('model'));
    }
    public function create()
    {
        $ImgMaxSizeModel = getImgMaxSizeModel();
        $nextSerialOrder = getNextSerialOrder(getSelectedValue(), 'Clients');
        $model = PageCategory::where('PagCat_Name', 'Clients')->where('PagCat_Status', '0')->orderBy('PagCat_Id', 'desc')->get();
        return view('backend.admin.clients.create', compact('nextSerialOrder', 'model','ImgMaxSizeModel'));
    }
    public function store(Request $request)
    {
        try {
            ...
            $model->save();
            return redirect()->route('admin.clients.create')->with('success', 'Data added successfully.');
        } catch (Exception $e) {
    ...
    public function edit($hashedId)
    {
        ...
        return view('backend.admin.clients.edit', compact('model','ImgMaxSizeModel'));
    }
    ...
    public function update(Request $request, $Pag_id)
    {
        try {
            ...
            $this->deleteOldImages($oldPag_Image, $model->Pag_Image);


            return redirect()->route('admin.clients.index')->with('success', 'Updated successfully.');
        } catch (Exception $e) {
            session()->flash('sticky_error', $e->getMessage());
            return back();
        }
    ...
    public function active($id)
    {
        try {
            ...
            return redirect()->route('admin.clients.index')->with('success', 'Clients marked as inactive.');
        } catch (\Exception $e) {
            return back()->with('error', 'Error: ' . $e->getMessage());
        }
    }
    public function inactive($id)
    {
        try {
            ...
            return redirect()->route('admin.clients.index')->with('success', 'Clients marked as active.');
        } catch (\Exception $e) {
            return back()->with('error', 'Error: ' . $e->getMessage());
        }
    }
