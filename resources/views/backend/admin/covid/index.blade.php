        ...
        <div class="app-page-title">
            <div class="page-title-wrapper d-flex justify-content-between align-items-center">
                <div class="page-title-heading">
                    ...
                    <h5 class="py-3">
                        <span class="text-muted fw-light">Covid /</span> List
                    </h5>
                </div>

                <div class="page-title-actions">
                    <a href="{{ URL::to('admin/covid/create') }}" class="btn btn-primary">
                        <i class="bi bi-plus-lg"></i> Add Covid
                    </a>
                </div>
            </div>
        </div>
                        ...
                        @foreach ($model as $value)
                            @if (Auth::user()->registration->Reg_Role === 'SUPERADMIN' ||
                                ...
                                <td class="serial-number"> {{ $loop->iteration }}</td>
                                    <td class="serial-number"> {{ $value->Pag_FullDesc }}</td>

                                    <td class="d-flex">
                                        <a href="{{ route('admin.covid.edit', encodeId($value->Pag_Id)) }}"
                                        ...
                                        <span class='me-2'>
                                            @if ($value->Pag_Status == 0)
                                                <a class="btn btn-success"
                                                    href="{{ URL::to('admin/covid/active', $value->Pag_Id) }}"><i
                                                        class="fa fa-check-circle active"></i> </a>
                                            @elseif ($value->Pag_Status == 1)
                                                <a class="btn btn-danger"
                                                    href="{{ URL::to('admin/covid/inactive', $value->Pag_Id) }}"><i
                                        ...
                                        <form action=" {{ route('admin.covid.destroy', $value->Pag_Id) }}"
