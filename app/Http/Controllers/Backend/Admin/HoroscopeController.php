...
class HoroscopeController extends Controller
{
    ...
    {
        $model = Page::where('Pag_Status', '!=', 2)->where('Pag_Reg_Id', getSelectedValue())->with('category')->whereHas('category', fn($query) => $query->where('PagCat_Name', 'Horoscope'))->orderBy('Pag_SerialOrder', 'asc')->get();
        // \Log::info('Current order:', $model->pluck('Pag_SerialOrder')->toArray());
        return view('backend.admin.horoscope.index', compact('model'));
    }
    ...
    public function create()
    {
        ...
        $nextSerialOrder = getNextSerialOrder(getSelectedValue(), 'Horoscope');
        $model = PageCategory::where('PagCat_Name', 'Horoscope')->where('PagCat_Status', '0')->orderBy('PagCat_Id', 'desc')->get();

        return view('backend.admin.horoscope.create', compact('nextSerialOrder', 'model','ImgMaxSizeModel'));
    }
    public function store(Request $request)
    {
        try {
            ...
            $model->save();
            return redirect()->route('admin.horoscope.create')->with('success', 'Data added successfully.');
        } catch (Exception $e) {
    ...
    //     return view('backend.admin.horoscope.edit', compact('model'));
    // }
    ...
    public function edit($hashedId)
    {
        try {
            ...
            $model = Page::where('Pag_id', $Pag_id)->first();
            return view('backend.admin.horoscope.edit', compact('model','ImgMaxSizeModel'));
        } catch (\Exception $e) {
    ...
    public function update(Request $request, $Pag_id)
    {
        try {
            ...
            return redirect()->route('admin.horoscope.index')->with('success', ' Updated successfully.');
        } catch (Exception $e) {
            session()->flash('sticky_error', $e->getMessage());
            return back();
        }
    ...
    public function active($id)
    {
        try {
            ...
            return redirect()->route('admin.horoscope.index')->with('success', 'Horoscope marked as inactive.');
        } catch (\Exception $e) {
            return back()->with('error', 'Error: ' . $e->getMessage());
        }
    }
    public function inactive($id)
    {
        try {
            ...
            return redirect()->route('admin.horoscope.index')->with('success', 'Horoscope marked as active.');
        } catch (\Exception $e) {
            return back()->with('error', 'Error: ' . $e->getMessage());
        }
    }
