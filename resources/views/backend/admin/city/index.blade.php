        ...
        <div class="app-page-title">
            <div class="page-title-wrapper d-flex justify-content-between align-items-center">
                ...
                <div class="page-title-actions">
                    <a href="{{ URL::to('admin/city/create') }}" class="btn btn-primary">
                    ...
                    <tbody class="DragNdDrop">
                        @foreach ($model as $value)
                            @if (Auth::user()->registration->Reg_Role === 'SUPERADMIN' ||
                                    (!$value->Cit_AdminExists && Auth::user()->role !== 'SUPERADMIN'))
                                <tr data-id="{{ $value->Cit_Id }}">
                                    ...
                                    <td> {{ $value->Cit_Code }} </td>
                                    <td class="d-flex"> <a href="{{ route('admin.city.edit', encodeId($value->Cit_Id)) }}"
                                            class="btn btn-primary me-2">
                                            <i class="fa fa-pencil"></i>
                                        </a>
                                        <span class='me-2'>
                                            @if ($value->Cit_Status == 0)
                                                <a class="btn btn-success"
                                                    href="{{ URL::to('admin/city/active', $value->Cit_Id) }}"><i
                                                        class="fa fa-check-circle active"></i>
                                                </a>
                                            @elseif ($value->Cit_Status == 1)
                                                <a class="btn btn-danger"
                                                    href="{{ URL::to('admin/city/inactive', $value->Cit_Id) }}"><i
                                        ...
                                        <form action=" {{ route('admin.city.destroy', $value->Cit_Id) }}" method="POST"
                                            id="deleteForm">
                                            @method('DELETE')
                                            @csrf
